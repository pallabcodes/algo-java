...
public class MaxSumSubArray {
    public static void main(String[] args) {
        ...
        // System.out.println(Arrays.toString(targetsSum(new int[]{-1, 2, 3, 4, 8, 9}, 7)));
    }
    ...
    private static int maxSumSubArr(int[] a, int k) {
        ...
        // sliding logic : remove the first windows from previous window & add the current item to match the current total
    ...
    // Q: find the length of maximum subarray where all elements are same (hint: sliding window)
    private static int maxSubArrSameElem(int[] a) {
        int n = 0;
        int l = 0; // comparison

        // main concept: arr1 = [4 4 4 ] [ 4 5 5 ]
        // when at the first element then current value and l is same, at the next iteration
        // if the value is same : then no need to do anything as l already has that value so just increase length
        // but if value isn't the same: if the current subarray has no same element then store the current value. increase length
    ...
    // Q: Given a sorted, find indices of two element whose sum is same as the target value (assume there's only solution)
    ...
    private static int[][] targetsSum(int[] a, int t) {
        ...
        while (start <= end) {
            if (a[start] + a[end] > t) {
                ...
                // now here make ArrayList or List to store the result from the "list" then add the others to that 2d array
                // to find all other occurrence of two indices that is same as target value then use two pointer again
                start = start + 1;
                end = end - 1;
                while (start <= end) {
                    if (a[start] + a[end] < t) {
